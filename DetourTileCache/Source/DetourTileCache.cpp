...
dtStatus dtTileCache::addObstacle(const Fix16* pos, const Fix16 radius, const Fix16 height, dtObstacleRef* result)
{
...
dtStatus dtTileCache::addBoxObstacle(const Fix16* bmin, const Fix16* bmax, dtObstacleRef* result)
{
	if (m_nreqs >= MAX_REQUESTS)
		return DT_FAILURE | DT_BUFFER_TOO_SMALL;
...
dtStatus dtTileCache::addBoxObstacle(const Fix16* center, const Fix16* halfExtents, const Fix16 yRadians, dtObstacleRef* result)
{
	if (m_nreqs >= MAX_REQUESTS)
		return DT_FAILURE | DT_BUFFER_TOO_SMALL;
...
dtStatus dtTileCache::queryTiles(const Fix16* bmin, const Fix16* bmax,
...
dtStatus dtTileCache::update(const Fix16 /*dt*/, dtNavMesh* navmesh,
							 bool* upToDate)
{
	if (m_nupdate == 0)
	{
