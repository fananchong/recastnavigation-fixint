...
inline bool overlapSlabs(const Fix16* amin, const Fix16* amax,
						 ...
						 const Fix16 px, const Fix16 py)
{
	// Check for horizontal overlap.
	// The segment is shrunken a little so that slabs which touch
	// at end points are not connected.
	const Fix16 minx = dtMax(amin[0]+px,bmin[0]+px);
	const Fix16 maxx = dtMin(amax[0]-px,bmax[0]-px);
	if (minx > maxx)
		return false;
	
	// Check vertical overlap.
	const Fix16 ad = (amax[1]-amin[1]) / (amax[0]-amin[0]);
	const Fix16 ak = amin[1] - ad*amin[0];
	const Fix16 bd = (bmax[1]-bmin[1]) / (bmax[0]-bmin[0]);
	const Fix16 bk = bmin[1] - bd*bmin[0];
	const Fix16 aminy = ad*minx + ak;
	const Fix16 amaxy = ad*maxx + ak;
	const Fix16 bminy = bd*minx + bk;
	const Fix16 bmaxy = bd*maxx + bk;
	const Fix16 dmin = bminy - aminy;
	const Fix16 dmax = bmaxy - amaxy;
...
static Fix16 getSlabCoord(const Fix16* va, const int side)
{
	if (side == 0 || side == 4)
		return va[0];
...
static void calcSlabEndPoints(const Fix16* va, const Fix16* vb, Fix16* bmin, Fix16* bmax, const int side)
{
	if (side == 0 || side == 4)
	{
		if (va[2] < vb[2])
...
//////////////////////////////////////////////////////////////////////////////////////////
int dtNavMesh::findConnectingPolys(const Fix16* va, const Fix16* vb,
								   const dtMeshTile* tile, int side,
								   dtPolyRef* con, Fix16* conarea, int maxcon) const
{
	if (!tile) return 0;
	
	Fix16 amin[2], amax[2];
	calcSlabEndPoints(va, vb, amin, amax, side);
	const Fix16 apos = getSlabCoord(va, side);
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		for (int j = 0; j < nv; ++j)
		{
			...
			const Fix16* vc = &tile->verts[poly->verts[j]*3];
			const Fix16* vd = &tile->verts[poly->verts[(j+1) % nv]*3];
			const Fix16 bpos = getSlabCoord(vc, side);
...
//		unsigned short m = DT_EXT_LINK | (unsigned short)side;
		...
		for (int j = 0; j < nv; ++j)
		{
			...
			// Create new links
			const Fix16* va = &tile->verts[poly->verts[j]*3];
			const Fix16* vb = &tile->verts[poly->verts[(j+1) % nv]*3];
			dtPolyRef nei[4];
			Fix16 neia[4*2];
			...
			for (int k = 0; k < nnei; ++k)
			{
				...
				if (idx != DT_NULL_LINK)
				{
					...
					if (dir == 0 || dir == 4)
					{
						Fix16 tmin = (neia[k*2+0]-va[2]) / (vb[2]-va[2]);
					...
					{
						Fix16 tmin = (neia[k*2+0]-va[0]) / (vb[0]-va[0]);
...
void dtNavMesh::connectExtOffMeshLinks(dtMeshTile* tile, dtMeshTile* target, int side)
{
	...
	for (int i = 0; i < target->header->offMeshConCount; ++i)
	{
		...
		const Fix16 halfExtents[3] = { targetCon->rad, target->header->walkableClimb, targetCon->rad };
		
		// Find polygon to connect to.
		const Fix16* p = &targetCon->pos[3];
		Fix16 nearestPt[3];
		...
		Fix16* v = &target->verts[targetPoly->verts[1]*3];
		dtVcopy(v, nearestPt);
...
void dtNavMesh::baseOffMeshLinks(dtMeshTile* tile)
{
	...
	for (int i = 0; i < tile->header->offMeshConCount; ++i)
	{
		...
		const Fix16 halfExtents[3] = { con->rad, tile->header->walkableClimb, con->rad };
		
		// Find polygon to connect to.
		const Fix16* p = &con->pos[0]; // First vertex
		Fix16 nearestPt[3];
		dtPolyRef ref = findNearestPolyInTile(tile, p, halfExtents, nearestPt);
		if (!ref) continue;
		// findNearestPoly may return too optimistic results, further check to make sure. 
		if (dtSqr(nearestPt[0]-p[0])+dtSqr(nearestPt[2]-p[2]) > dtSqr(con->rad))
			continue;
		// Make sure the location is on current mesh.
		Fix16* v = &tile->verts[poly->verts[0]*3];
		dtVcopy(v, nearestPt);
...
void dtNavMesh::closestPointOnPoly(dtPolyRef ref, const Fix16* pos, Fix16* closest, bool* posOverPoly) const
{
...
dtPolyRef dtNavMesh::findNearestPolyInTile(const dtMeshTile* tile,
										   const Fix16* center, const Fix16* halfExtents,
										   Fix16* nearestPt) const
{
	Fix16 bmin[3], bmax[3];
	...
	dtPolyRef nearest = 0;
	Fix16 nearestDistanceSqr = FLT_MAX;
...
int dtNavMesh::queryPolygonsInTile(const dtMeshTile* tile, const Fix16* qmin, const Fix16* qmax,
							...
							dtTileRef lastRef, dtTileRef* result)
{
	...
	const int vertsSize = dtAlign4(sizeof(Fix16)*3*header->vertCount);
	const int polysSize = dtAlign4(sizeof(dtPoly)*header->polyCount);
	const int linksSize = dtAlign4(sizeof(dtLink)*(header->maxLinkCount));
	const int detailMeshesSize = dtAlign4(sizeof(dtPolyDetail)*header->detailMeshCount);
	const int detailVertsSize = dtAlign4(sizeof(Fix16)*3*header->detailVertCount);
	const int detailTrisSize = dtAlign4(sizeof(unsigned char)*4*header->detailTriCount);
	const int bvtreeSize = dtAlign4(sizeof(dtBVNode)*header->bvNodeCount);
	const int offMeshLinksSize = dtAlign4(sizeof(dtOffMeshConnection)*header->offMeshConCount);
	
	unsigned char* d = data + headerSize;
	tile->verts = dtGetThenAdvanceBufferPointer<Fix16>(d, vertsSize);
	...
	tile->detailVerts = dtGetThenAdvanceBufferPointer<Fix16>(d, detailVertsSize);
...
void dtNavMesh::calcTileLoc(const Fix16* pos, int* tx, int* ty) const
{
...
dtStatus dtNavMesh::getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, Fix16* startPos, Fix16* endPos) const
{
	unsigned int salt, it, ip;
