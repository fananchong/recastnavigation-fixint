...
static int createBVTree(dtNavMeshCreateParams* params, dtBVNode* nodes, int /*nnodes*/)
{
	...
	for (int i = 0; i < params->polyCount; i++)
	{
		...
		if (params->detailMeshes)
		{
			...
			const Fix16* dv = &params->detailVerts[vb*3];
			dtVcopy(bmin, dv);
			dtVcopy(bmax, dv);
...
static unsigned char classifyOffMeshPoint(const Fix16* pt, const Fix16* bmin, const Fix16* bmax)
{
...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	if (params->offMeshConCount > 0)
	{
		...
		if (params->detailVerts && params->detailVertsCount)
		{
			...
			{
				const Fix16 h = params->detailVerts[i*3+1];
				hmin = dtMin(hmin,h);
				hmax = dtMax(hmax,h);
			}
		}
		...
		{
			const Fix16* p0 = &params->offMeshConVerts[(i*2+0)*3];
			const Fix16* p1 = &params->offMeshConVerts[(i*2+1)*3];
	...
	const int vertsSize = dtAlign4(sizeof(Fix16)*3*totVertCount);
	const int polysSize = dtAlign4(sizeof(dtPoly)*totPolyCount);
	const int linksSize = dtAlign4(sizeof(dtLink)*maxLinkCount);
	const int detailMeshesSize = dtAlign4(sizeof(dtPolyDetail)*params->polyCount);
	const int detailVertsSize = dtAlign4(sizeof(Fix16)*3*uniqueDetailVertCount);
	...
	Fix16* navVerts = dtGetThenAdvanceBufferPointer<Fix16>(d, vertsSize);
	...
	Fix16* navDVerts = dtGetThenAdvanceBufferPointer<Fix16>(d, detailVertsSize);
	...
	for (int i = 0; i < params->offMeshConCount; ++i)
	{
		...
		if (offMeshConClass[i*2+0] == 0xff)
		{
			const Fix16* linkv = &params->offMeshConVerts[i*2*3];
			Fix16* v = &navVerts[(offMeshVertsBase + n*2)*3];
			dtVcopy(&v[0], &linkv[0]);
			dtVcopy(&v[3], &linkv[3]);
			n++;
		}
	...
	if (params->detailMeshes)
	{
		...
		for (int i = 0; i < params->polyCount; ++i)
		{
			...
			if (ndv-nv)
			{
				memcpy(&navDVerts[vbase*3], &params->detailVerts[(vb+nv)*3], sizeof(Fix16)*3*(ndv-nv));
	...
	for (int i = 0; i < params->offMeshConCount; ++i)
	{
		...
		if (offMeshConClass[i*2+0] == 0xff)
		{
			...
			const Fix16* endPts = &params->offMeshConVerts[i*2*3];
...
bool dtNavMeshDataSwapEndian(unsigned char* data, const int /*dataSize*/)
{
	...
	Fix16* verts = dtGetThenAdvanceBufferPointer<Fix16>(d, vertsSize);
	...
	Fix16* detailVerts = dtGetThenAdvanceBufferPointer<Fix16>(d, detailVertsSize);
