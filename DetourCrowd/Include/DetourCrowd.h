...
struct dtCrowdNeighbour
{
	...
	Fix16 dist;		///< The distance between the current agent and the neighbor.
};
...
{
	Fix16 radius;						///< Agent radius. [Limit: >= 0]
	Fix16 height;						///< Agent height. [Limit: > 0]
	Fix16 maxAcceleration;				///< Maximum allowed acceleration. [Limit: >= 0]
	Fix16 maxSpeed;						///< Maximum allowed speed. [Limit: >= 0]

	/// Defines how close a collision element must be before it is considered for steering behaviors. [Limits: > 0]
	Fix16 collisionQueryRange;

	Fix16 pathOptimizationRange;		///< The path visibility optimization range. [Limit: > 0]

	/// How aggresive the agent manager should be at avoiding collisions with this agent. [Limit: >= 0]
	Fix16 separationWeight;
...
struct dtCrowdAgent
{
	...
	Fix16 npos[3];		///< The current agent position. [(x, y, z)]
	Fix16 disp[3];		///< A temporary value used to accumulate agent displacement during iterative collision resolution. [(x, y, z)]
	Fix16 dvel[3];		///< The desired velocity of the agent. Based on the current path, calculated from scratch each frame. [(x, y, z)]
	Fix16 nvel[3];		///< The desired velocity adjusted by obstacle avoidance, calculated from scratch each frame. [(x, y, z)]
	Fix16 vel[3];		///< The actual velocity of the agent. The change from nvel -> vel is constrained by max acceleration. [(x, y, z)]

	/// The agent's configuration parameters.
	dtCrowdAgentParams params;

	/// The local path corridor corners for the agent. (Staight path.) [(x, y, z) * #ncorners]
	Fix16 cornerVerts[DT_CROWDAGENT_MAX_CORNERS*3];
	...
	Fix16 targetPos[3];					///< Target position of the movement request (or velocity in case of DT_CROWDAGENT_TARGET_VELOCITY).
	dtPathQueueRef targetPathqRef;		///< Path finder ref.
	bool targetReplan;					///< Flag indicating that the current path is being replanned.
	Fix16 targetReplanTime;				/// <Time since the agent's target was replanned.
};

struct dtCrowdAgentAnimation
{
	bool active;
	Fix16 initPos[3], startPos[3], endPos[3];
	dtPolyRef polyRef;
	Fix16 t, tmax;
};
...
class dtCrowd
{
	...
	Fix16 m_agentPlacementHalfExtents[3];

	dtQueryFilter m_filters[DT_CROWD_MAX_QUERY_FILTER_TYPE];

	Fix16 m_maxAgentRadius;

	int m_velocitySampleCount;

	dtNavMeshQuery* m_navquery;

	void updateTopologyOptimization(dtCrowdAgent** agents, const int nagents, const Fix16 dt);
	void updateMoveRequest(const Fix16 dt);
	void checkPathValidity(dtCrowdAgent** agents, const int nagents, const Fix16 dt);

	inline int getAgentIndex(const dtCrowdAgent* agent) const  { return (int)(agent - m_agents); }

	bool requestMoveTargetReplan(const int idx, dtPolyRef ref, const Fix16* pos);
...
public:
	...
	bool init(const int maxAgents, const Fix16 maxAgentRadius, dtNavMesh* nav);
	...
	int addAgent(const Fix16* pos, const dtCrowdAgentParams* params);
	...
	bool requestMoveTarget(const int idx, dtPolyRef ref, const Fix16* pos);

	/// Submits a new move request for the specified agent.
	///  @param[in]		idx		The agent index. [Limits: 0 <= value < #getAgentCount()]
	///  @param[in]		vel		The movement velocity. [(x, y, z)]
	/// @return True if the request was successfully submitted.
	bool requestMoveVelocity(const int idx, const Fix16* vel);
	...
	void update(const Fix16 dt, dtCrowdAgentDebugInfo* debug);
	...
	const Fix16* getQueryHalfExtents() const { return m_agentPlacementHalfExtents; }

	/// Same as getQueryHalfExtents. Left to maintain backwards compatibility.
	/// @return The search halfExtents used by the crowd. [(x, y, z)]
	const Fix16* getQueryExtents() const { return m_agentPlacementHalfExtents; }
