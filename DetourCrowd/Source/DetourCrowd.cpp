...
static void integrate(dtCrowdAgent* ag, const Fix16 dt)
{
	...
	const Fix16 maxDelta = ag->params.maxAcceleration * dt;
	Fix16 dv[3];
...
static bool overOffmeshConnection(const dtCrowdAgent* ag, const Fix16 radius)
{
	if (!ag->ncorners)
		return false;
	...
	{
		const Fix16 distSq = dtVdist2DSqr(ag->npos, &ag->cornerVerts[(ag->ncorners-1)*3]);
...
static Fix16 getDistanceToGoal(const dtCrowdAgent* ag, const Fix16 range)
{
	if (!ag->ncorners)
		return range;
...
static void calcSmoothSteerDirection(const dtCrowdAgent* ag, Fix16* dir)
{
	if (!ag->ncorners)
	{
		dtVset(dir, 0,0,0);
	...
	const Fix16* p0 = &ag->cornerVerts[ip0*3];
	const Fix16* p1 = &ag->cornerVerts[ip1*3];
...
static void calcStraightSteerDirection(const dtCrowdAgent* ag, Fix16* dir)
{
	if (!ag->ncorners)
	{
		dtVset(dir, 0,0,0);
...
static int addNeighbour(const int idx, const Fix16 dist,
...
static int getNeighbours(const Fix16* pos, const Fix16 height, const Fix16 range,
						 ...
						 dtCrowdAgent** agents, const int /*nagents*/, dtProximityGrid* grid)
{
	...
	for (int i = 0; i < nids; ++i)
	{
		...
		if (dtMathFabsf(diff[1]) >= (height+ag->params.height)/2.0f)
			continue;
		diff[1] = 0;
		const Fix16 distSqr = dtVlenSqr(diff);
...
bool dtCrowd::init(const int maxAgents, const Fix16 maxAgentRadius, dtNavMesh* nav)
{
	purge();
...
int dtCrowd::addAgent(const Fix16* pos, const dtCrowdAgentParams* params)
{
	// Find empty slot.
	int idx = -1;
	for (int i = 0; i < m_maxAgents; ++i)
...
bool dtCrowd::requestMoveTargetReplan(const int idx, dtPolyRef ref, const Fix16* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
bool dtCrowd::requestMoveTarget(const int idx, dtPolyRef ref, const Fix16* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
	if (!ref)
...
bool dtCrowd::requestMoveVelocity(const int idx, const Fix16* vel)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
void dtCrowd::updateMoveRequest(const Fix16 /*dt*/)
{
...
void dtCrowd::updateTopologyOptimization(dtCrowdAgent** agents, const int nagents, const Fix16 dt)
{
	if (!nagents)
		return;
	
	const Fix16 OPT_TIME_THR = 0.5f; // seconds
...
void dtCrowd::checkPathValidity(dtCrowdAgent** agents, const int nagents, const Fix16 dt)
{
	static const int CHECK_LOOKAHEAD = 10;
	static const Fix16 TARGET_REPLAN_DELAY = 1.0; // seconds
...
void dtCrowd::update(const Fix16 dt, dtCrowdAgentDebugInfo* debug)
{
	m_velocitySampleCount = 0;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		const Fix16 updateThr = ag->params.collisionQueryRange*0.25f;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		{
			const Fix16* target = &ag->cornerVerts[dtMin(1,ag->ncorners-1)*3];
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		// Check 
		const Fix16 triggerRadius = ag->params.radius*2.25f;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		else
		{
			...
			const Fix16 slowDownRadius = ag->params.radius*2;	// TODO: make less hacky.
			const Fix16 speedScale = getDistanceToGoal(ag, slowDownRadius) / slowDownRadius;
	...
	// Handle collisions.
	static const Fix16 COLLISION_RESOLVE_FACTOR = 0.7f;
	...
	for (int iter = 0; iter < 4; ++iter)
	{
		for (int i = 0; i < nagents; ++i)
		{
			...
			for (int j = 0; j < ag->nneis; ++j)
			{
				...
				Fix16 dist = dtVlenSqr(diff);
				if (dist > dtSqr(ag->params.radius + nei->params.radius))
					continue;
				dist = dtMathSqrtf(dist);
				Fix16 pen = (ag->params.radius + nei->params.radius) - dist;
				if (dist < 0.0001f)
				{
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		// Update position
		const Fix16 ta = anim->tmax*0.15f;
		const Fix16 tb = anim->tmax;
		if (anim->t < ta)
		{
			const Fix16 u = tween(anim->t, 0.0, ta);
			dtVlerp(ag->npos, anim->initPos, anim->startPos, u);
		}
		else
		{
			const Fix16 u = tween(anim->t, ta, tb);
