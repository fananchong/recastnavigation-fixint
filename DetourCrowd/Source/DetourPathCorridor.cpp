...
void dtPathCorridor::reset(dtPolyRef ref, const Fix16* pos)
{
	dtAssert(m_path);
	dtVcopy(m_pos, pos);
...
*/
int dtPathCorridor::findCorners(Fix16* cornerVerts, unsigned char* cornerFlags,
							  ...
							  dtNavMeshQuery* navquery, const dtQueryFilter* /*filter*/)
{
	...
	static const Fix16 MIN_TARGET_DIST = 0.01f;
	...
	while (ncorners)
	{
		...
		if (ncorners)
		{
			...
			memmove(cornerVerts, cornerVerts+3, sizeof(Fix16)*3*ncorners);
		}
	}
...
*/
void dtPathCorridor::optimizePathVisibility(const Fix16* next, const Fix16 pathOptimizationRange,
										  dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	...
	Fix16 goal[3];
	dtVcopy(goal, next);
	Fix16 dist = dtVdist2D(m_pos, goal);
...
*/
bool dtPathCorridor::movePosition(const Fix16* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	dtAssert(m_npath);
...
*/
bool dtPathCorridor::moveTargetPosition(const Fix16* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	dtAssert(m_npath);
...
void dtPathCorridor::setCorridor(const Fix16* target, const dtPolyRef* path, const int npath)
{
	dtAssert(m_path);
	dtAssert(npath > 0);
...
bool dtPathCorridor::fixPathStart(dtPolyRef safeRef, const Fix16* safePos)
{
	dtAssert(m_path);
...
bool dtPathCorridor::trimInvalidPath(dtPolyRef safeRef, const Fix16* safePos,
									 dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(navquery);
	dtAssert(filter);
